...
static void update_callback(lv_display_t *disp)
{
    ...
    ESP_UTILS_LOGD("Update display rotation to %d", lv_display_get_rotation(disp));
...
static lv_display_t *display_init(LCD *lcd)
{
    ...
    lv_display_set_buffers(disp,lvgl_buf[0],lvgl_buf[1],buffer_size*lv_color_format_get_size(lv_display_get_color_format(disp)),LV_DISPLAY_RENDER_MODE_PARTIAL);
...
static void touchpad_read(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    int read_touch_result = tp->readPoints(&point, 1, 2);
