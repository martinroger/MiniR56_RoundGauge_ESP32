...
// Parse a CAN Frame if available
    ...
    if ((millis() - lastDispValuesRefreshed) > DISP_VALUES_REFRESH_INTERVAL && lvgl_port_lock(2))
    {
    ...
    if (!screenON)
    {
        ...
        {
            active_board->getBacklight()->setBrightness(tempbrightness * 100 / 255);
